...
struct TweetService {
    ...
    func uploadTweet(caption: String, type: UploadTweetConfiguration, completion: @escaping(DatabaseCompletion)) {
        ...
        switch type {
        case .tweet:
            // Update user-tweet structure after tweet upload completes. This fastens searching for tweets of particular user. Helps in ProfileController.
    ...
    func fetchTweets(completion: @escaping ([Tweet]) -> Void) {
        ...
        REF_USER_FOLLOWING.child(currentUid).observe(.childAdded) { snapshot in
            let follwingUid = snapshot.key
            
            REF_USER_TWEETS.child(follwingUid).observe(.childAdded) { snapshot in
    ...
    func fetchLikes(forUser user: User, completion: @escaping ([Tweet]) -> Void) {
        ...
        REF_USER_LIKES.child(user.uid).observe(.childAdded) { snapshot in
            let tweedID = snapshot.key
            self.fetchTweet(withTweetID: tweedID) { likedTweet in
                var tweet = likedTweet
                tweet.didLike = true // to ensure proper button look
                tweets.append(tweet)
                completion(tweets)
            }
        }
